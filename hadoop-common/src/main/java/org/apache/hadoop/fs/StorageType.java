...
public enum StorageType {
  ...
  /*,RAM_DISK,其实就是Memory内存,而ARCHIVE并没有特指哪种存储介质,
   * 主要的指的是高密度存储数据的介质来解决数据量的容量扩增的问题.
   * true或者false代表的是此类存储类型是否是transient特性的.
   * transient的意思是指转瞬即逝的,并非持久化的.
   * 没有主动声明数据目录存储类型的,默认都是DISK.*/
  RAM_DISK(true),
  SSD(false),
  DISK(false),
  ARCHIVE(false);
